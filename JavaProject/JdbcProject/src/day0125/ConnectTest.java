...
public class ConnectTest {
	...
	public void connectSawon() {
		...
		try {
			...
			// rs=DriverManager.getConnection(URL, "stu",
			// "a1234").createStatement().executeQuery(sql);

			// 여러줄을 가져올때는 while문으로 가져온다
			// rs.next():다음데이터로 이동하면서 true반환,더이상데이터 없으면 false반환
			while (rs.next()) {
				// db로 부터 데이터 가져오기
	...
	public void sawonRead() {
		...
		String sql = "select buseo,count(*) cnt,round(avg(pay),2) pay from sawon group by buseo";
